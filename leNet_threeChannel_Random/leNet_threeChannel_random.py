  ...
  with tf.Session() as sess:
    ...
    with tf.name_scope('accuracy'):
      with tf.name_scope('correct_prediction'):
        ...
        # correct_train_color_prediction = tf.equal(tf.argmax(train_color_prediction, 1), train_colors_node) #color
      with tf.name_scope('accuracy'):
        label_accuracy = tf.reduce_mean(tf.cast(correct_train_label_prediction, tf.float32))
        # color_accuracy = tf.reduce_mean(tf.cast(correct_train_color_prediction, tf.float32)) #color
      tf.scalar_summary('label_accuracy', label_accuracy) #!!!
      # tf.scalar_summary('color_accuracy', color_accuracy) #!!!#color
    ...
    for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        # print('Minibatch color error: %.1f%%' % error_rate(color_predictions, batch_colors))#color
        ...
        # print('Validation color error: %.1f%%' % error_rate(eval_color_predictions, validation_colors))#color
        sys.stdout.flush()
    ...
    # print('Test color error: %.1f%%' % test_color_error) #color
    if FLAGS.self_test:
